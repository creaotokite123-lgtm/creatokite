...
router.get('/submissions', auth, async (req, res) => {
  try {
    const q = req.user.role === 'creator' ? { creator: req.user._id } : {};
...
router.get('/academy/lessons', auth, async (req, res) => {
  try {
    const lessons = await Lesson.find().sort({ category: 1, sortOrder: 1 });
    
    // Fetch completed lessons for the user
    const completed = await LessonCompletion.find({ creator: req.user._id });
    const completedSet = new Set(completed.map(c => c.lesson.toString()));
...
/* GET /api/admin/submissions — List pending submissions for review */
router.get('/admin/submissions', auth, adminOnly, async (req, res) => {
  try {
    const submissions = await Submission.find({ status: 'pending' })
      .populate('activity')
      .populate('creator', 'displayName email avatar niche')
      .sort({ createdAt: 1 });
    res.json({ success: true, submissions });
  } catch (e) {
    res.status(500).json({ success: false, message: e.message });
